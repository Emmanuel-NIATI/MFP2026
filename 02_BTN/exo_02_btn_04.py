...
def ctrl_led(color: str):
    if color == COLOR_R:
        ...
        if state_led_red:
            led_red.blink(TIME_HIGH_MEDIUM,TIME_LOW_MEDIUM , None, True)
        else:
            led_red.off()
    elif color == COLOR_G:
    ...
    if color == COLOR_B:
        ...
        if state_led_blue:
            led_blue.blink(TIME_HIGH_MEDIUM,TIME_LOW_MEDIUM , None, True)
        else:
            led_blue.off()
    elif color == COLOR_W:
        ...
        if state_led_white:
            led_white.blink(TIME_HIGH_MEDIUM,TIME_LOW_MEDIUM , None, True)
        else:
            led_white.off()

def setup():
    print("Exercice 02 - Les boutons - Partie 04")
